...
const dog = () =>
  fetch("dogs").then((response) => {
    const dogsArr = response.data.dogs;
    const naughtyDogIndex = dogsArr.findIndex((dog) => {
      return dog.naughty === 10;
    });
    return dogsArr[naughtyDogIndex];
  });
...
const joke = () => {
  let obj = {};
  const q = fetch("jokes", "question");
  const a = fetch("jokes");
  return Promise.all([q, a]).then((response) => {
    console.log(response);
    obj.question = response[0].joke;
    obj.answer = response[1].answer;
    console.log(obj);
    return obj;
  });
};
